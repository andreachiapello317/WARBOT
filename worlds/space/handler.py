...
from worlds.space.menu import menu_keyboard, menu_text, results_keyboard
from worlds.space.queries import get_query, run_query
from worlds.space.service import format_space
...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not city:
        ...
        return
    await show_world_menu(update, context, world_id="space", menu_text=menu_text(city), menu_keyboard=menu_keyboard())
...
async def show_query(update: Update, context: ContextTypes.DEFAULT_TYPE, query_id: str) -> None:
    ...
    await run_world_query(
        ...
        query_id=query_id,
        run=run_query,
        format_result=format_space,
        wait_text=f"⏳ SPACE · {city_label(city)}…",
